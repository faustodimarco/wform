...
/**
* ErrorHandler - Error handling utility for Wform
*/
class ErrorHandler {
  static log(message, type = 'log') {
    console[type](`[Wform] ${message}`);
  }
  static warn(message) {
  ...
  static showInputError(input, message) {
    try {
      ...
      if (errorElement) {
        ...
        const target = (wrapper || input.parentElement);
        target.appendChild(newErrorElement);
      }
...
/**
* DOMUtils - DOM utility functions for Wform
*/
class DOMUtils {
...
/**
* ValidationRules - Validation rules for Wform
*/
class ValidationRules {
  static validateEmail(value) {
    if (!value || value.trim() === '') return true; // Empty non-required field is valid
  ...
  static validate(input, required = false) {
    const type = input.type;
    const value = input.value;
    
    // First check if required field is empty
    if (required && !this.validateRequired(type === 'checkbox' ? input.checked : value)) {
      return {
        valid: false,
    ...
    switch (type) {
      ...
      case 'number':
        return {
          valid: this.validateNumber(value),
          message: 'Please enter a valid number'
        };
      case 'checkbox':
        return {
          valid: this.validateCheckbox(input.checked, required),
      ...
      case 'radio':
        ...
        return {
          valid: this.validateRadioGroup(group, required),
          message: 'Please select an option'
        };
      case 'select-one':
      case 'select-multiple':
        return {
          valid: this.validateSelect(value, required),
          message: 'Please select an option'
        };
      default:
        // For any other input type, use the native validation if available
...
/**
* Step - Step management for Wform
*/
class Step {
...
/**
* Validator - Validation engine for Wform
*/
class Validator {
  ...
  validateInput(input) {
    try {
      // Skip disabled and hidden inputs
      ...
      // Use our custom validation rules
      const validationResult = ValidationRules.validate(input, isRequired);
      
      if (validationResult.valid) {
        ErrorHandler.clearInputError(input);
      } else {
        ErrorHandler.showInputError(input, validationResult.message);
      }
      
      // Trigger custom validation event
      this.eventEmitter.emit('input:validate', {
        input: input,
  ...
  updateStepValidity(step) {
    try {
      ...
      const requiredValid = requiredInputs.length === 0 || 
                          requiredInputs.every(input => ValidationRules.validate(input, true).valid);
      
      // Then validate all non-required inputs that have values
      const allInputs = step.getAllInputs();
      const nonRequiredValid = allInputs
        .filter(input => !input.required && !input.disabled && input.type !== 'hidden')
        .every(input => ValidationRules.validate(input, false).valid);
      
      const isValid = requiredValid && nonRequiredValid;
      step.setValidity(isValid);
    } catch (error) {
...
/**
* PhoneFormatter - Phone number formatting for Wform
*/
class PhoneFormatter {
  ...
  static initializeAll(form) {
    try {
      const phoneInputs = document.querySelectorAll('input[data-phone-autoformat], input[type="tel"]');
      phoneInputs.forEach(input => {
        const format = input.getAttribute('data-phone-autoformat') || 'xxx-xxx-xxxx';
...
/**
* EmailValidator - Email validation for Wform
*/
class EmailValidator {
  ...
  validateInput(showError = false) {
    try {
      ...
      if (!value && isRequired) {
        if (showError) {
          ErrorHandler.showInputError(this.input, 'This field is required');
        }
        return false;
      }
...
/**
* Navigation - Multi-step navigation for Wform
*/
class Navigation {
  constructor(form, eventEmitter) {
    this.form = form;
...
/**
* ProgressIndicator - Progress indicator management for Wform
*/
...
/**
* Form - Form management for Wform
*/
class Form {
...
class Wform {
  ...
  init() {
    try {
      if (this.initialized) {
        ErrorHandler.warn('Wform already initialized');
        return;
      }
      this.findForms();
      ...
      this.initialized = true;
      this.eventEmitter.emit('Wform:initialized', { form: this.currentForm });
      ErrorHandler.log('Wform initialized successfully', 'info');
    } catch (error) {
      ErrorHandler.error(`Error initializing Wform: ${error.message}`);
      console.error(error);
    }
  }
  findForms() {
...
// Initialize Wform when DOM is loaded
document.addEventListener('DOMContentLoaded', () => {
  window.Wform = new Wform();
  window.Wform.init();
});
